...
Interaction Bilingualism:Ethnicity:Gender is significant, so break down by Gender 
```{r}
###Analysis in Female
model_F <- lm(Executive_Function ~ Bilingualism * Ethnicity + poly(Age,2)  +  EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Female'))
car::Anova(model_F)
eta_squared(model_F)
effectsize(model_F)
```
...
```{r}
### Analysis in males
model_M <- lm(Executive_Function ~ Bilingualism * Ethnicity  + poly(Age,2)+  EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Male'))
car::Anova(model_M)  
effectsize(model_M)
eta_squared(model_M)
```
...
Interaction Bilingualism:Age:Gender is significant, so break down by Gender 
```{r}
###Analysis in Female
