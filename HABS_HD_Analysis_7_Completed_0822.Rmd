...
model_cognitive <- glm(PD ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + Bilingualism * Ethnicity + EducationYears, data = HABS_HD_cross_sectional, family = binomial)
...
```{r}
## Analysis in Black
model_B <- glm(PD ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='Black'), family = binomial)
car::Anova(model_B)
effectsize(model_B)  
...
```{r}
## Analysis in White
model_W <- glm(PD ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='White'), family = binomial)
car::Anova(model_W) 
effectsize(model_W)
...
model_H <- glm(PD ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='Hispanic'), family = binomial)
car::Anova(model_H) 
effectsize(model_H)
...
plot_data  <- fot.fun(data=HABS_HD_cross_sectional, "PD", c("Bilingualism","Ethnicity"))

plot_data <- subset(plot_data, PD >= 0 & PD <= 1)

p1<-ggplot(plot_data, aes(x = Bilingualism, y = PD, fill = Bilingualism)) +
  ...
  xlab("Ethnicity") +
  ggtitle("Probability of Potential Dementia (PD) by Bilingualism across Ethic Groups") +
...
# Run PCA
pca_result <- psych::principal(efa_data, nfactors = 3, rotate = "oblimin")
# View results
...
```{r}
three_factor_efa_fit = efa(data = efa_data, nfactors=3, missing="fiml", rotation = "oblimin")
summary(three_factor_efa_fit, fit.measures = TRUE, standardized = TRUE)
predicted_scores <- lavPredict(three_factor_efa_fit)
predicted_scores <- as.data.frame(predicted_scores)
colnames(predicted_scores) <- c("Executive_Function", "Working_Memory", "Episodic_Memory")
HABS_HD_cross_sectional <- cbind(HABS_HD_cross_sectional, predicted_scores)
```

We conducted exploratory factor analysis with 3 factors to identify the latent structure of the cognitive battery and to guide confirmatory modelling

### 1.2.2 Analysis on Cognitive Performance: Executive_Function domain 
```{r}
HABS_HD_cross_sectional$Ethnicity<-relevel(HABS_HD_cross_sectional$Ethnicity, ref="Hispanic")
a1<-lm(Executive_Function ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + Bilingualism * Ethnicity + EducationYears, data = HABS_HD_cross_sectional)
car::Anova(a1)
summary(a1)
effectsize(a1)
eta_squared(a1)
...
```{r}
###analysis in Hispanic
model_H <- lm(Executive_Function ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='Hispanic'))
car::Anova(model_H) 
effectsize(model_H)
eta_squared(model_H)
```

Bilingual Hispanics outperformed monolingual Hispanics
...
The main effect of bilingualism is not significant but the interaction Bilingualism: Gender is significant. So we breakdown the interaction within White people

```{r}
### analysis in White females
model_WF <- lm(Executive_Function ~ Bilingualism * poly(Age,2)  +  EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='White'& Gender =="Female"))
car::Anova(model_WF) 
effectsize(model_WF)
eta_squared(model_WF)
```

No bilingual effect in white females

```{r}
### analysis in White males
model_WM <- lm(Executive_Function ~ Bilingualism * poly(Age,2)  +  EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='White'& Gender =="Male"))
car::Anova(model_WM)  
effectsize(model_WM)
eta_squared(model_WM)
```

NO bilingual effect in White males

Interaction between Bilingualism and Gender is significant, so break down by Gender 
```{r}
###Analysis in Female
model_F <- lm(Executive_Function ~ Bilingualism * poly(Age,2) + Bilingualism * Ethnicity +  EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Female'))
car::Anova(model_F)
effectsize(model_F)
eta_squared(model_F)
```
...
```{r}
### Analysis in males
model_M <- lm(Executive_Function ~ Bilingualism * poly(Age,2) + Bilingualism * Ethnicity +  EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Male'))
car::Anova(model_M)  
effectsize(model_M)
eta_squared(model_M)
```
...
```{r}
plot_data  <- fot.fun(data=HABS_HD_cross_sectional, "Executive_Function", c("Bilingualism","Ethnicity","Gender"))
...
### 1.2.3 Analysis on Cognitive Performance: Working memory 
```{r}
HABS_HD_cross_sectional$Ethnicity<-relevel(HABS_HD_cross_sectional$Ethnicity, ref="White")
a2 <- lm(Working_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + Bilingualism * Ethnicity + EducationYears, data = HABS_HD_cross_sectional)  
summary(a2)
car::Anova(a2) 
effectsize(a2)
eta_squared(a2)
...
The main effect of Ethnicity is significant: Black and White outperformed Hispanic people.

### 1.2.4 Analysis on Cognitive Performance: Episodic Memory
```{r}
HABS_HD_cross_sectional$Ethnicity<-relevel(HABS_HD_cross_sectional$Ethnicity, ref="Black")
a3 <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + Bilingualism * Ethnicity + EducationYears, data = HABS_HD_cross_sectional)  
car::Anova(a3)  
effectsize(a3)
eta_squared(a3)
```
...
```{r}
###Analysis in Black
model_B <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='Black'))
car::Anova(model_B)  
effectsize(model_B)
eta_squared(model_B)
```
...
```{r}
### Analysis in Hispanic
model_H <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='Hispanic'))
car::Anova(model_H)
effectsize(model_H)
eta_squared(model_H)
```
...
```{r}
model_W <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Gender + EducationYears, data = subset(HABS_HD_cross_sectional, Ethnicity=='White'))
car::Anova(model_W) 
effectsize(model_W)
eta_squared(model_W)
```
...
```{r}
###Analysis in Females
model_F <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Ethnicity + EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Female'))
car::Anova(model_F)  
effectsize(model_F)
eta_squared(model_F)
```

Interaction between Bilingualism and Ethnicity is significant in females, so breakdown the interaction

```{r}
### Analysis in Hispanic females
model_FH <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + EducationYears, data = subset(HABS_HD_cross_sectional, Gender == "Female" & Ethnicity=='Hispanic'))
car::Anova(model_FH)
effectsize(model_H)
eta_squared(model_H)
```
...
```{r}
### Analysis in Black females
model_FB <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + EducationYears, data = subset(HABS_HD_cross_sectional, Gender == "Female" & Ethnicity=='Black'))
car::Anova(model_FB)
effectsize(model_FB)
eta_squared(model_FB)
```

No bilingual effect in Black females

```{r}
### Analysis in White females
model_FW <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + EducationYears, data = subset(HABS_HD_cross_sectional, Gender == "Female" & Ethnicity=='White'))
car::Anova(model_FW)
effectsize(model_FW)
...
```{r}
###Analysis in Males
model_M <- lm(Episodic_Memory ~ Bilingualism * poly(Age,2) + Bilingualism * Ethnicity + EducationYears, data = subset(HABS_HD_cross_sectional, Gender=='Male'))
car::Anova(model_M)
effectsize(model_M)
eta_squared(model_M)
```

No bilingual effect in males

```{r}
plot_data  <- fot.fun(data=HABS_HD_cross_sectional, "Episodic_Memory", c("Bilingualism","Ethnicity","Gender"))
p3<-ggplot(plot_data, aes(x = Ethnicity, y = Episodic_Memory, fill = Bilingualism)) +
  geom_col(position = position_dodge(0.9), width = 0.7) +
  ...
  xlab("Ethnicity") +
  ggtitle("Episodic Memory by Bilingualism Across Ethnic and Gender Groups") +
  scale_fill_brewer(palette = "Set1") +
  theme_bw() +
  scale_y_continuous(labels = percent_format(accuracy = 1)) + 
  theme(
...
```{r}
HABS_HD_cross_sectional_English$Ethnicity<-relevel(HABS_HD_cross_sectional_English$Ethnicity, ref="White")
model_English <- lm(AMNART ~ Bilingualism2 * poly(Age,2) + Bilingualism2 * Gender + Bilingualism2 * Ethnicity + EducationYears, data = HABS_HD_cross_sectional_English)
...
model_B <- lm(AMNART ~ Bilingualism2 * poly(Age,2) + Bilingualism2 * Gender + EducationYears, data = subset(HABS_HD_cross_sectional_English, Ethnicity=='Black'))
car::Anova(model_B)
effectsize(model_B)
eta_squared(model_B)
```
...
```{r}
model_H <- lm(AMNART ~ Bilingualism2 * poly(Age,2) + Bilingualism2 * Gender + EducationYears, data = subset(HABS_HD_cross_sectional_English, Ethnicity=='Hispanic'))
car::Anova(model_H)
effectsize(model_H)
eta_squared(model_H)
```
...
```{r}
model_W <- lm(AMNART ~ Bilingualism2 * poly(Age,2) + Bilingualism2 * Gender + EducationYears, data = subset(HABS_HD_cross_sectional_English, Ethnicity=='White'))
car::Anova(model_W)
effectsize(model_W)
eta_squared(model_W)
```
...
```{r}
plot_data  <- fot.fun(HABS_HD_cross_sectional_English, "AMNART", c("Bilingualism2","Ethnicity"))

ggplot(plot_data, aes(x = Bilingualism2, y = AMNART, fill = Bilingualism2)) +
  geom_col(position = position_dodge(0.9), width = 0.7) +
  geom_errorbar(aes(ymin = AMNART - se, ymax = AMNART + se),
                width = 0.2,
                position = position_dodge(0.9)) +
  facet_wrap(~Ethnicity) +
  ylab("AMNART performance (Z-scored)") +
  xlab("Bilingualism") +
...
model_spanish<-lm(WAT ~  Bilingualism2 * poly(Age,2) + Bilingualism2 * Gender + EducationYears, HABS_HD_WAT)
...
```{r}
# Run PCA
pca_result <- psych::principal(efa_data2, nfactors = 3, rotate = "oblimin")
# View results
print(pca_result, digits = 2, cutoff = 0.3)
pca_result$Vaccounted
```
...
```{r}
three_factor_efa_fit2 = lavaan::efa(data =efa_data2, nfactors=3, rotation='oblimin', missing="fiml")
summary(three_factor_efa_fit2, fit.measures = TRUE, standardized = TRUE)

predicted_scores2 <- lavPredict(three_factor_efa_fit2)
predicted_scores2 <- as.data.frame(predicted_scores2)
colnames(predicted_scores2) <- c("Executive_Function", "Working_Memory", "Episodic_Memory")
...
```{r}
a1<-lmer(Executive_Function ~  A0 * dA * Bilingualism + dA * Bilingualism * Gender + dA * Bilingualism * Ethnicity + EducationYears + Practice + (1|Med_ID), data = HABS_HD_longitudinal)
car::Anova(a1)
effectsize(a1)
eta_squared(a1)
```
...
### 2.2.2 Analysis on Cognitive Performance: Short-term Memory (STM)
```{r}
a2<-lmer(Working_Memory ~  A0 * dA * Bilingualism + dA * Bilingualism * Gender + dA * Bilingualism * Ethnicity + EducationYears + Practice + (1|Med_ID), data = HABS_HD_longitudinal)
car::Anova(a2) 
effectsize(a2)
eta_squared(a2)
```
...
### 2.2.3 Analysis on Cognitive Performance: Long-term Memory (Episodic_Memory)
```{r}
a3<-lmer(Episodic_Memory ~  A0 * dA * Bilingualism + dA * Bilingualism * Gender + dA * Bilingualism * Ethnicity + EducationYears + Practice + (1|Med_ID), data = HABS_HD_longitudinal)
car::Anova(a3) 
effectsize(a3)
eta_squared(a3)
```
...
```{r}
plot_spaghetti <- function(data, x_var, y_var, x_label, y_label, ymin, ymax, color_choice) {
  ggplot(data, aes(x = .data[[x_var]], y = .data[[y_var]], color = Bilingualism)) +
   geom_point(size = 2, stroke = 1, alpha = 0.1, color = color_choice) +
    ...
    labs(x = x_label, y = y_label, color = "Bilingualism") +
    ylim(ymin, ymax) +
    theme_minimal() + 
         #facet_grid( Ethnicity ~ Bilingualism) + 
...
p5<-plot_spaghetti(HABS_HD_longitudinal, "Age", "Episodic_Memory","Age","Episodic_Memory Changes across Visits",min(HABS_HD_longitudinal$Episodic_Memory), max(HABS_HD_longitudinal$Episodic_Memory),  color_choice = "darkorange")

ggsave("Episodic_Memory_longitudinal.png", plot = p5, 
       width = 8, height = 6, units = "in", 
       dpi = 800) 
